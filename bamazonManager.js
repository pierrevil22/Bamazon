...
function displayInventory() {
	...
	connection.query(queryStr, function(err, data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			strOut += 'Department: ' + data[i].department_id + '  //  ';
		...
		connection.query(queryStr, {id: item}, function(err, data) {
			...
			if (data.length === 0) {
				...
				connection.query(updateQueryStr, function(err, data) {
					...
					console.log('Stock count for ID ' + item + ' has been updated to ' + (productData.stock_quantity + addQuantity) + '.');
					console.log("\n---------------------------------------------------------------------\n");
...
function createNewProduct() {
	...
	inquirer.prompt([
		...
		console.log('Adding New Item: \n    product_name = ' + input.product_name + '\n' +  
									   '    department_name = ' + input.department_id + '\n' +  
									   '    price = ' + input.price + '\n' +  
